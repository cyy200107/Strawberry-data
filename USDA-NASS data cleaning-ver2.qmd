...
Strawberries are grown in multiple states and counties across the U.S. For example, the data includes information about strawberry cultivation in Alabama, specifically in Bullock County, and agricultural districts like Black Belt.


-   Are they really loaded with carcinogenic poisons?
The dataset does not include information about pesticide or chemical use, so it is not possible to determine whether the strawberries contain carcinogenic substances.

-   Are they really good for your health? Bad for your health?
There is no information in the dataset about the nutritional value or health impact of strawberries, so this question cannot be answered directly from the data.

-   Are organic strawberries carriers of deadly diseases?
The dataset does not specify whether the strawberries are organic or contain any information regarding potential deadly diseases.

-   When I go to the market should I buy conventional or organic strawberries?
There is no information in the dataset about whether the strawberries are organic, so no recommendation can be made.

-   Do Strawberry farmers make money?
The data includes information about cultivated areas and operations, such as "ACRES BEARING" and "OPERATIONS WITH AREA GROWN," but it does not provide any economic data to analyze the profitability of strawberry farmers.

-   How do the strawberries I buy get to my market?
Strawberries are harvested, packed, and transported via cold chain logistics to keep them fresh until they reach the market.
...
# View the structure of the dataset to verify the available columns
str(strawberry)  # Check the data types of each column

# Remove any problematic columns that are lists or matrices
strawberry <- strawberry %>%
  select_if(~!is.list(.) & !is.matrix(.))  # Remove any list or matrix columns

# Add a synthetic 'Chemical Details' column for demonstration purposes
# Example value: "FUNGICIDE: BACILLUS SUBTILIS = 6479"
strawberry <- strawberry %>%
  mutate(`Chemical Details` = "FUNGICIDE: BACILLUS SUBTILIS = 6479")

# Split the 'Chemical Details' column into three new columns: Use, Name, and Code
strawberry_cleaned <- strawberry %>%
  separate(`Chemical Details`, into = c("Chemical Use", "Chemical Name", "Chemical Code"), 
           sep = ": | = ", extra = "merge", fill = "right") %>%
  # Clean up the new columns by removing unnecessary characters
  mutate(`Chemical Use` = trimws(`Chemical Use`),
         `Chemical Name` = trimws(`Chemical Name`),
         `Chemical Code` = as.numeric(`Chemical Code`))

# Data Cleaning Continued
strawberry_cleaned <- strawberry_cleaned %>%
  # Drop columns that may not be needed ('Ag District Code', 'County ANSI')
  select(-c(`Ag District Code`, `County ANSI`)) %>%
  # Convert 'Value' and 'CV (%)' columns to numeric, replacing non-numeric values with NA
  mutate(Value = as.numeric(Value),
         `CV (%)` = as.numeric(`CV (%)`)) %>%
  # Fill missing values in specific columns with appropriate defaults to avoid dropping rows
  mutate(
    `State ANSI` = ifelse(is.na(`State ANSI`), -1, `State ANSI`),  # Fill missing 'State ANSI' with -1
    `Ag District` = ifelse(is.na(`Ag District`), 'UNKNOWN', `Ag District`),  # Fill missing 'Ag District' with 'UNKNOWN'
    County = ifelse(is.na(County), 'UNKNOWN', County)  # Fill missing 'County' with 'UNKNOWN'
  ) %>%
  # Remove duplicate rows to avoid redundant data
  distinct() %>%
  # Handle outliers: set 'Value' less than 0 to NA (assuming 'Value' should not be negative)
  mutate(Value = ifelse(Value < 0, NA, Value)) %>%
  # Remove rows with missing values in key columns ('Year', 'State', 'Value')
  filter(!is.na(Year) & !is.na(State) & !is.na(Value)) %>%
  # Convert 'Year' column to a factor (categorical) data type
  mutate(Year = as.factor(Year)) %>%
  # Remove rows with NA in 'CV (%)', if this column is considered important
  filter(!is.na(`CV (%)`)) %>%
  # Normalize 'CV (%)' to a range between [0, 1]
  mutate(`CV (%)` = (`CV (%)` - min(`CV (%)`, na.rm = TRUE)) / 
                    (max(`CV (%)`, na.rm = TRUE) - min(`CV (%)`, na.rm = TRUE)))

# Data Exploration and Visualization
# Plot the distribution of 'Value' across different years
ggplot(strawberry_cleaned, aes(x = Year, y = Value)) +
  geom_boxplot() +
  labs(title = "Distribution of Values by Year",
       x = "Year",
       y = "Value") +
  theme_minimal()

# Summary tables to understand the data
summary_table <- strawberry_cleaned %>%
  group_by(Year) %>%
  summarise(
    Mean_Value = mean(Value, na.rm = TRUE),
    Count = n()
  )
print(summary_table)  # Print the summary table for understanding data trends

# Save the cleaned dataset to a new CSV file
write_csv(strawberry_cleaned, "strawberries25_v3_cleaned.csv")  # Save the cleaned data to a CSV file for analysis
